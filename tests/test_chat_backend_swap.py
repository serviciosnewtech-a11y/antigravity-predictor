...
default, but a client must be able to swap /api/chat to a different agent
without touching predictor_server.py. Covers: auto-detect priority order
(hermes_proxy -> anthropic -> ollama), CHAT_BACKEND forcing exactly one
backend and refusing to silently fail over to another, and the native
Anthropic Messages API call (a genuinely different wire shape from the
OpenAI-compatible proxy path: system is a top-level param not a "system"
role, auth is x-api-key not Bearer, reply is content[0]["text"] not
...
def test_auto_detect_prefers_hermes_proxy_over_anthropic_and_ollama(monkeypatch):
    ...
    monkeypatch.setattr(ps, "requests", type("R", (), {"post": staticmethod(fake_post)}))

    cfg = ps._backend_config()
    result = ps._call_llm_backend("system prompt", "hello", [], cfg)
...
def test_chat_backend_override_forces_anthropic_even_with_proxy_set(monkeypatch):
    ...
    def fake_post(url, json=None, headers=None, **kwargs):
        ...
        # Anthropic's shape: system is top-level, not a "system"-role message.
...
def test_chat_backend_override_misconfigured_does_not_fall_through(monkeypatch):
    # Client explicitly asked for anthropic but never set the key -- should
    # come back unavailable, NOT silently try ollama even if it's set.
    _clear_backend_env(monkeypatch)
    monkeypatch.setenv("OLLAMA_URL", "http://fake-ollama:1234")
